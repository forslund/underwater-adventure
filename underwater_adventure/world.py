...
class Weld(Thing):
    def use(self, target):
        if target.name == 'knife':
            self.gettable = True
            self.description = """An old Lincoln Welding machine. It's in surprisingly good condition for a
thing that's been submerged for this long. Unfortunately it's not usuable in water."""
            return "Using the table knife the screws are removed one by one until the weld is free to be moved."
        else:
...
thing that's been submerged for this long. Unfortunately it's not usuable in water. It has been secured to a wall using a couple of flat head screws."""
...
class OxygenTube(Thing):
    def use(self, target):
        if target.name == 'start' or target == hatch:
            ...
            hatch.key(weld)
            return """The oxygen is released an form an air pocket around the hatch."""
...
outside = Room()
outside.description = """The sun glimers far above, you are finally free.

As you slowly asend towards the surface you look down onto the ship that
trapped you and caused such claustrophobia. From the outside it looks quite
serene.

You tear your eyes from it and focus on the ever closer surface, a few bubbles
escape as you relieved laugh into your mask.

...
"""
start_room = Room('start')
start_room.description = """It is dark but  A Bluish light filters in through a
small round window. The room is colorless and the walls seem to be made of
metal."""

corridor = Room()
corridor.description = """The corridor is dark, with very little decoration.
All along the corridors are bulkheads similar to the one you entered through.
At the far end there is a stairwell.
"""

captains_quarters = Room('captain')
captains_quarters.description = """A sligthly bigger room than the ones below
deck. with a bolted down desk centrally located. Debris on the floor looks like
they once were books, now a fragile gray mass.

In the ceiling there is a small pocket off air.
"""
...
steering_hut = Room()
steering_hut.description = """By the look of it this was the bridge, was being
the operative word the cramped space has been crushed down. The rear of the
...
galley.description = """The ship's galley, the floor is covered by debris,
 but along the wall pots and cantines are still standing even if they are
 in a state of disarray. The stove is rusted and has likely fried it's last
 egg. The tubing for the gas is the only thing that looks to be in decent
 shape.
"""
...
darkness = Room()
darkness.description = """It is dark, you will likely need some sort of light to explore further
